#pragma once
#include "com_string.hpp"
#include <cstring>
#include <algorithm>

namespace Engine
{
	void StringUtil::SplitC(const std::string& str, const std::string delim, std::list<std::string>& result)
	{
		char* p = strtok_s(const_cast<char*>(str.c_str()), delim.c_str(), nullptr);
		while (p)
		{
			std::string s = p;
			result.emplace_back(s);
			p = strtok_s(nullptr, delim.c_str(), nullptr);
		}
	}

	void StringUtil::SplitCpp(const std::string& str, const std::string delim, std::list<std::string>& result)
	{
		if (str.empty()) {
			return;
		}
	...
	void StringUtil::Trim(std::list<std::string>* str_list) {
		if (NULL == str_list) {
			return;
		}
	...
	void StringUtil::UrlEncode(const std::string& src_str, std::string* dst_str)
	{
		dst_str->clear();
	...
	void StringUtil::UrlDecode(const std::string& src_str, std::string* dst_str)
	{
		dst_str->clear();
	...
	bool StringUtil::Hex2Bin(const char* hex_str, std::string* bin_str)
	{
...
#define ASCII2DEC(c) \
	...
	bool StringUtil::Bin2Hex(const char* bin_str, std::string* hex_str)
	{
